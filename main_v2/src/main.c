...
//#define TX_BUFFER_SIZE 1024     // For responses (UART1, UART2)
//#define SPECTRUM_DATA_SIZE 512  // FPGA spectrum data size
//#define MCP4725_ADDRESS 0x60    // MCP4725 I2C address
//char buffer[64];

#define RX_BUFFER_SIZE 256      // For user input (UART1)
#define TX_BUFFER_SIZE 8192     // Increased for larger UART2 packets (was 1024)
#define SPECTRUM_DATA_SIZE 4096 // Updated to 4K channels (was 512)
#define MCP4725_ADDRESS 0x60    // MCP4725 I2C address
char buffer[128]; // Increased buffer size for longer log messages
...
static volatile uint32_t packetStartTime = 0; // For timeout
static const uint32_t PACKET_TIMEOUT = 5000; // 2 seconds timeout
    ...
    if (uart1RxComplete) {
        ...
        if (strcmp((char *)uart1RxBuffer, "START") == 0) {
            ...
            if (!isAcquisitionRunning) {
                ...
                UART2_ReadAbort();
                while (UART2_ReadCountGet()) UART2_Read(uart2RxBuffer, TX_BUFFER_SIZE);
                memset(uart2RxBuffer, 0, TX_BUFFER_SIZE);
                ...
                if (UART2_Write(startCmd, 3)) {
                    ...
                    delay_cycles(DELAY_1MS_CYCLES * 2000);
                    UART2_Read(uart2RxBuffer, TX_BUFFER_SIZE);
                } else {
            ...
            if (UART2_Write(resetCmd, 3)) {
                ...
                delay_cycles(DELAY_1MS_CYCLES * 200);
                UART2_Read(uart2RxBuffer, TX_BUFFER_SIZE);
            } else {
